...
def bayes_preference_loss(batch: Dict[str, Union[List, torch.LongTensor]],
    ...
    """Compute the Bayes-DPO loss for a batch of policy and reference model log probabilities.
    ...
    kernel_matrix[:batch_size,batch_size:] = similarity_2.reshape(batch_size,batch_size)
        ...
        if loss_config.name in {'dpo', 'ipo'}:
            ...
            if train_test == 'train':
                losses, chosen_rewards, rejected_rewards, kl = bayes_preference_loss(
                    batch, policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps, reference_chosen_hidden, reference_rejected_hidden ,self.all_additional_hps, train_process=True, **loss_kwargs)
            else:
                losses, chosen_rewards, rejected_rewards, kl = bayes_preference_loss(
                    batch, policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps, reference_chosen_hidden, reference_rejected_hidden ,self.all_additional_hps, train_process=False, **loss_kwargs)
