...
def bayes_preference_loss(batch: Dict[str, Union[List, torch.LongTensor]],
    ...
    for chosen_item in batch["chosen_response_only"]:
        for chosen_item_ in batch["chosen_response_only"]:
            list1.append(chosen_item)
            list2.append(chosen_item_)

    for chosen_item in batch["chosen_response_only"]:
        for rejected_item in batch["rejected_response_only"]:
            list1.append(chosen_item)
            list2.append(rejected_item)

    for rejected_item in batch["rejected_response_only"]:
        for chosen_item in batch["chosen_response_only"]:
            list1.append(rejected_item)
            list2.append(chosen_item)
